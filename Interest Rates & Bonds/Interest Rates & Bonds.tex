...
Drawbacks of linear interpolation are that the zero curve so obtained is not smooth., 
...
last term also includes the present value of the face value which is returned in the end.
...
\end{equation}
Convexity is positive for vanilla bonds. 

Convexity reduces the decrease in bond price and increases
the increase in bond price.

A higher convexity offers dual sided benefits and thus makes the bond costlier.
\section*{Callable Bond}
\sep
A callable bond is bond with optionality, the issuer can recall it and settle the bond at the
\textbf{call price} before a specified period of time. 

A callable bond has a lower duration
than a normal bond. This is because, as interest rates fall, the issuer may recall it and settle
the bond.

Callable bonds can have negative convexity. 

Price of a callable bond tends to be capped at the call price.
\end{document}
